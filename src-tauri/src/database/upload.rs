...
#[cfg(test)]
pub fn get_last_upload_at_at_path(db_path: &PathBuf) -> Result<String, String> {
    let conn = open_from_path(db_path)?;
    let mut stmt = conn
        .prepare("SELECT value FROM metadata WHERE key='last_upload_at' LIMIT 1")
        .map_err(|e| e.to_string())?;
...
#[cfg(test)]
pub fn select_pending_rows_at_path(db_path: &PathBuf, max_rows: usize) -> Result<Vec<UploadRow>, String> {
    let conn = open_from_path(db_path)?;
    let watermark = get_last_upload_at_at_path(db_path)?;
    let mut stmt = conn
        .prepare(
            "SELECT id, table_name, json, run_id, created_at \
             FROM results \
...
#[cfg(test)]
pub fn mark_rows_handled_and_advance_watermark_at_path(db_path: &PathBuf, ids: &[i64]) -> Result<(), String> {
    if ids.is_empty() { return Ok(()); }
    let mut conn = open_from_path(db_path)?;
    let tx = conn.transaction().map_err(|e| e.to_string())?;
    let (ph, params_vec) = build_in_clause_params(ids);
