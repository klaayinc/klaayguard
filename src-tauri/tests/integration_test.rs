...
fn test_jwt_token_validation() {
    ...
    let invalid_tokens = vec!["not.a.jwt.token", "only.two", "single", ""];
...
fn test_deep_link_url_schemes() {
    ...
    for url in valid_urls {
        assert!(
            url.starts_with("klaayguard://"),
            "Should accept klaayguard:// scheme: {}",
            url
        );
    }
    ...
    for url in invalid_urls {
        assert!(
            !url.starts_with("klaayguard://"),
            "Should reject non-klaayguard:// scheme: {}",
            url
        );
    }
}
